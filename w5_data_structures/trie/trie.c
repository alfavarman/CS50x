...
int main(int argc, char *argv[])
{
    ...
    while (fscanf(infile, "%s", name) == 1)
    {
        ...
        for (int i = 0, n = strlen(name); i < n; i++)
        {
            ...
            // int index = hashing_index(name[i])
...
bool check(char* word)
{
    // set cursor/ptr at root.
    node *cursor = root;

    // for each letter at len(word)
    for (int i = 0, n = strlen(word); i < n; i++)
    {
        // get letter index
        int index = hashing_index(word[i]);


        // if there io children at this index means its not in base.
        if (cursor->children[index] == NULL)
        {
            return false;
        }

        else if (i == n - 1 && cursor->children[index]->is_word)
        {
            return true;
        }
        cursor = cursor->children[index];
